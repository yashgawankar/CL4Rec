...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='DIGINETICA', type=str)  # name of dataset in ['DIGINETICA', 'YOOCHOOSE', 'MOVIELENS']
    parser.add_argument('--save_dir', default='ADER', type=str)  # name of dictionary save the results
    # exemplar
    parser.add_argument('--exemplar_size', default=20000, type=int)  # size of exemplars
    ...
    for period in periods:
        ...
        # load test data
        test_sess = []
        for test_period in range(period):
          test_sess_period, info = dataloader.evaluate_loader(test_period)
          test_sess.append(test_sess_period)
        ...
        with tf.Session(config=config) as sess:
            ...
            for test_sess_period in test_sess: 
              print("\n\ntest ", test_num)
              
              test_evaluator = Evaluator(test_sess_period, False, args.maxlen, args.test_batch,
                                        max_item, 'test', model, sess)
              info = test_evaluator.evaluate(best_epoch)
              print('period ' + str(test_num) + ' ' + info + '\n')
              logs.write('period ' + str(test_num) + ' ' + info + '\n')
              test_num+=1
              mrr_20_tmp.append(test_evaluator.results()[0])
              recall_20_tmp.append(test_evaluator.results()[1])
              mrr_10_tmp.append(test_evaluator.results()[2])
              recall_10_tmp.append(test_evaluator.results()[3])
            ...
            logs.write('current period ' + str(period) + ' mrr@20 = ' + str(np.mean(mrr_20_tmp)) + ' recall@20 = ' + str(np.mean(recall_20_tmp)) +' mrr@10 = ' + str(np.mean(mrr_10_tmp))  + ' recall@10 = ' + str(np.mean(recall_10_tmp)) +'\n')
            print('current period ' + str(period) + ' mrr@20 = ' + str(np.mean(mrr_20_tmp)) + ' recall@20 = ' + str(np.mean(recall_20_tmp)) +' mrr@10 = ' + str(np.mean(mrr_10_tmp))  + ' recall@10 = ' + str(np.mean(recall_10_tmp)) +'\n')
            # Select exemplars
